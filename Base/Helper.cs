...
namespace cotf.Base
{
	...
	public static class Drawing
	{
		...
		public class BitmapOptimization
		{
			public static void ProcessBitmap(Bitmap bitmap)
			{
				...
				Parallel.For(0, height, y =>
				{
					...
					for (int x = 0; x < width; x++)
					{
						...
						int index = yOffset + xOffset;

						// Example operation: Invert colors
			...
			public static Bitmap Lightpass0(List<Tile> brush, Bitmap bmp, Vector2 topLeft, Lamp light, float range)
			{
				...
				Parallel.For(0, height, y =>
				{
					...
					for (int x = 0; x < width; x++)
					{
						...
						float distance = (float)Helper.Distance(topLeft + new Vector2(x, y), light.position);
						float radius = Helper.NormalizedRadius(distance, range);
						if (radius > 0f && dynamic(brush, new Vector2(x, y), topLeft, light, range))
						{
							Color srcPixel = Color.FromArgb
							(
								pixels[index + 3],
								pixels[index + 2],
								pixels[index + 1],
							...
							Color result = Ext.Multiply(srcPixel, light.lampColor, radius);

							pixels[index] = result.B;		// Blue
							pixels[index + 1] = result.G; // Green
							pixels[index + 2] = result.R; // Red
							pixels[index + 3] = result.A; // Alpha
						}
					}
				});
